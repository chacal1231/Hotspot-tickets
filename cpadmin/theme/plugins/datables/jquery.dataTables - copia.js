...
(/** @lends <global> */function($, window, document, undefined) {
	...
	var DataTable = function( oInit )
	{
		...
		function _fnAddColumn( oSettings, nTh )
		{
			...
			if ( oSettings.aoPreBuscarCols[ iCol ] === undefined || oSettings.aoPreBuscarCols[ iCol ] === null )
			{
				oSettings.aoPreBuscarCols[ iCol ] = $.extend( {}, DataTable.models.oBuscar );
			}
			else
			{
				var oPre = oSettings.aoPreBuscarCols[ iCol ];
		 ...
		 *  @param {object} oOptions object with sType, bVisible and bBuscarable
		 *  @memberof DataTable#oApi
		 */
		...
		function _fnReDraw( oSettings )
		{
			if ( oSettings.oFeatures.bSort )
			{
				...
				_fnSort( oSettings, oSettings.oPreviousBuscar );
			}
			else if ( oSettings.oFeatures.bFilter )
			{
				/* Filtering will redraw for us */
				_fnFilterComplete( oSettings, oSettings.oPreviousBuscar );
			}
			else
			{
				_fnCalculateEnd( oSettings );
		...
		function _fnAjaxParameters( oSettings )
		{
			...
			{
				aoData.push( { "name": "sBuscar", "value": oSettings.oPreviousBuscar.sBuscar } );
				aoData.push( { "name": "bRegex",  "value": oSettings.oPreviousBuscar.bRegex } );
				for ( i=0 ; i<iColumns ; i++ )
				{
					aoData.push( { "name": "sBuscar_"+i,     "value": oSettings.aoPreBuscarCols[i].sBuscar } );
					aoData.push( { "name": "bRegex_"+i,      "value": oSettings.aoPreBuscarCols[i].bRegex } );
					aoData.push( { "name": "bBuscarable_"+i, "value": oSettings.aoColumns[i].bBuscarable } );
				}
			}
		...
		function _fnSort ( oSettings, bApplyClasses )
		{
			...
			if ( oSettings.oFeatures.bFilter )
			{
				...
				_fnFilterComplete( oSettings, oSettings.oPreviousBuscar, 1 );
			}
			else
			{
				oSettings.aiDisplay = oSettings.aiDisplayMaster.slice();
		...
		function _fnSaveState ( oSettings )
		{
			...
			var oState = {
				...
				"oBuscar":      $.extend( true, {}, oSettings.oPreviousBuscar ),
				"aoBuscarCols": $.extend( true, [], oSettings.aoPreBuscarCols ),
				"abVisCols":    []
			};
		...
		function _fnLoadState ( oSettings, oInit )
		{
			...
			/* Buscar filtering  */
			$.extend( oSettings.oPreviousBuscar, oData.oBuscar );
			$.extend( true, oSettings.aoPreBuscarCols, oData.aoBuscarCols );
		...
		this.fnDeleteRow = function( mTarget, fnCallBack, bRedraw )
		{
			...
			oSettings.asDataBuscar.splice( iDisplayIndex, 1 );
		...
		this.fnFilter = function( sInput, iColumn, bRegex, bSmart, bShowGlobal, bCaseInsensitive )
		{
			...
			else
			{
				...
				$.extend( oSettings.aoPreBuscarCols[ iColumn ], {
					"sBuscar": sInput+"",
					"bRegex": bRegex,
					"bSmart": bSmart,
					"bCaseInsensitive": bCaseInsensitive
		...
		this.fnUpdate = function( mData, mRow, iColumn, bRedraw, bAction )
		{
			...
			oSettings.asDataBuscar[iDisplayIndex] = _fnBuildBuscarRow( oSettings, 
		...
		this.oApi = {
			...
			"_fnFilter": _fnFilter,
			"_fnBuildBuscarArray": _fnBuildBuscarArray,
			"_fnBuildBuscarRow": _fnBuildBuscarRow,
			"_fnFilterCreateBuscar": _fnFilterCreateBuscar,
			"_fnDataToBuscar": _fnDataToBuscar,
			"_fnSort": _fnSort,
		...
		return this.each(function() {
			...
			_fnMap( oSettings, oInit, "oBuscar", "oPreviousBuscar" );
			_fnMap( oSettings, oInit, "aoBuscarCols", "aoPreBuscarCols" );
	...
	DataTable.models.ext = {
		 ...
		 * that is used for the column (if any). This is the corollary of <i>ofnBuscar</i> for sort 
		 * data.
		 *   <ul>
	     *     <li>
	     *       Function input parameters:
		 ...
		 *
		 *  @example
		 *    $.fn.dataTableExt.ofnBuscar['title-numeric'] = function ( sData ) {
		 *      return sData.replace(/\n/g," ").replace( /<.*?>/g, "" );
		 *    }
		 */
		"ofnBuscar": {},
	...
	DataTable.defaults = {
		...
		/**
		 * Basically the same as oBuscar, this parameter defines the individual column
		 * filtering state at initialisation time. The array must be of the same size 
		 * as the number of columns, and each element be an object with the parameters
		 * "sBuscar" and "bEscapeRegex" (the latter is optional). 'null' is also
		 ...
		 *          { "sBuscar": "My filter" },
		 *          null,
		 *          { "sBuscar": "^[0-9]", "bEscapeRegex": false }
		 *        ]
		 *      } );
		 *    } )
		 */
		 ...
		 *          alert( 'Saved filter was: '+oData.oFilter.sBuscar );
		 *      } );
		 *    } );
		 */
		"fnStateLoaded": null,
		...
		"oLanguage": {
			 ...
			 *      $('#example').dataTable( {
			 *        "oLanguage": {
			 *          "sInfo": "Got a total of _TOTAL_ Entradas to show (_START_ to _END_)"
			 *        }
			 *      } );
			 *    } );
			 */
			 ...
			 *      $('#example').dataTable( {
			 *        "oLanguage": {
			 *          "sInfoEmpty": "No Entradas to show"
			 *        }
			 *      } );
			 *    } );
			 */
			 ...
			 *      $('#example').dataTable( {
			 *        "oLanguage": {
			 *          "sBuscar": "Apply filter _INPUT_ to table"
			 *        }
			 *      } );
			 *    } );
			 */
		 ...
		 * initialisation time. As an object the "sBuscar" parameter must be
		 ...
		 *    $(document).ready( function() {
		 *      $('#example').dataTable( {
		 *        "oBuscar": {"sBuscar": "Initial search"}
		 *      } );
		 *    } )
		 */
	...
	DataTable.models.oSettings = {
		...
		/**
		 * Buscar data array for regular expression searching
		 *  @type array
		 *  @default []
		 */
		"asDataBuscar": [],
